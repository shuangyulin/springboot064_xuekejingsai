<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            ...
            <el-button
              v-if="isAuth('xuesheng','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="warning"
              icon="el-icon-s-data"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
            <el-button
              v-if="isAuth('xuesheng','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}<i class="el-icon-s-data el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('xuesheng','报表') && contents.btnAdAllIcon == 0"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
          </el-form-item>
    ...
    <el-dialog
      ...
      width="800">
        <div id="canyujingsaiChart" style="width:100%;height:600px;"></div>
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        var canyujingsaiChart = this.$echarts.init(document.getElementById("canyujingsaiChart"),'macarons');
        this.$http({
            url: "group/xuesheng/canyujingsai",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
            ...
            for(let i=0;i<res.length;i++){
                xAxis.push(res[i].canyujingsai);
                yAxis.push(res[i].total);
                pArray.push({
                value: res[i].total,
                name: res[i].canyujingsai
                })
                var option = {};
                option = {
                        title: {
                ...
                // 使用刚指定的配置项和数据显示图表。
                canyujingsaiChart.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    canyujingsaiChart.resize();
                };
            }
            }
        });
